...
class MockDataService {
  ...
  getCachedData() { return this.cached$.asObservable(); }
}
class MockUtilsService { sleep(ms: number) { return Promise.resolve(); } }
class MockNotification { errors: string[] = []; showError(m: string){ this.errors.push(m);} }
class MockRouter { navigated: any[] = []; navigate(a:any){ this.navigated.push(a);} }
...
describe('DataStateService', () => {
  ...
  function init(credsIn?: Partial<Credentials>){
    service.initializeState({
      plugin: 'git', pluginId: 'git', repo_name: 'r', user: 'u', url: '',
  dataset_id: 'doi:123', dataverse_token: 'tok',
      ...credsIn
    });
  }
  ...
  it('initializes and polls until ready true then stores result', fakeAsync(() => {
    ...
    data.cached$.next({ ready:true, res:{ data:[{ id:'b', name:'b', path:'', hidden:false }, { id:'a', name:'a', path:'', hidden:false }] } });
    tick();
  ...
  it('handles cached error and navigates', fakeAsync(() => {
    ...
    tick();
    expect(notify.errors.some(e=>e.includes('Comparing failed'))).toBeTrue();
