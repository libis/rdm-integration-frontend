...
`;
  ...
  it('should verify each LogicalDataSet has required properties', () => {
    ...
    logicalDataSetUris.forEach((uri) => {
      ...
      expect(identifiers.length)
        .withContext(`${uri} should have exactly one dcterms:identifier`)
        .toBe(1);
      ...
      expect(labels.length)
        .withContext(`${uri} should have exactly one skos:prefLabel`)
        .toBe(1);
      ...
      expect(descriptions.length)
        .withContext(`${uri} should have exactly one dcterms:description`)
        .toBe(1);
      ...
      expect(variables.length)
        .withContext(`${uri} should have at least one cdi:containsVariable`)
        .toBeGreaterThan(0);
    });
  });
  ...
  it('should parse SHACL shapes and verify LogicalDataSet shape properties', () => {
    ...
    expect(propertyPaths)
      .withContext(
        'LogicalDataSetShape should define dcterms:identifier property',
      )
    ...
    expect(propertyPaths)
      .withContext(
        'LogicalDataSetShape should define dcterms:description property',
      )
      .toContain('http://purl.org/dc/terms/description');
    expect(propertyPaths)
      .withContext(
        'LogicalDataSetShape should define cdi:containsVariable property',
      )
      .toContain(
        'http://www.ddialliance.org/Specification/DDI-CDI/1.0/RDF/containsVariable',
      );
  ...
  it('should demonstrate the issue: SHACL form needs shape properties to render fields', () => {
    console.log('\n=== ISSUE DEMONSTRATION ===');
    console.log(
      'The SHACL form component requires sh:property definitions in the shape',
    );
    ...
    console.log('');
    console.log(
      'The form will appear EMPTY even though the RDF data contains these values.',
    );
    console.log('');
    console.log(
      'This is because the SHACL form uses the shape definitions to determine:',
    );
    ...
    console.log('');
    console.log(
      'Solution: Add all three properties to LogicalDataSetShape in the SHACL template.',
    );
  });
});
