...
describe('FolderActionUpdateService', () => {
  ...
  function node(action: Fileaction, status: Filestatus): TreeNode<Datafile> {
    return {
      data: { id: status + ':' + action, name: '', path: '', hidden: false, action, status },
    } as TreeNode<Datafile>;
  }

  it('aggregates all equal -> Ignore', () => {
    const root: TreeNode<Datafile> = { data: { id: 'root', name: '', path: '', hidden: false }, children: [
  ...
  it('updateFoldersAction clears folder actions then computes', () => {
    ...
    // root folder
    map.set('', { data: { id: '', name: '', path: '', hidden: false }, children: [] });
    // file nodes with isFile attribute so they retain action
    const file1: TreeNode<Datafile> = { data: { id: 'f1', name: 'f1', path: '', hidden: false, action: Fileaction.Copy, status: Filestatus.New, attributes: { isFile: true } } };
    const file2: TreeNode<Datafile> = { data: { id: 'f2', name: 'f2', path: '', hidden: false, action: Fileaction.Ignore, status: Filestatus.Equal, attributes: { isFile: true } } };
