...
describe('DataStateService - 401 Auto-Reset', () => {
  describe('401 detection logic', () => {
    ...
    it('should verify the actual implementation handles falsy values correctly', () => {
      ...
      const is401_1 =
        error1.status === 401 || (error1.error && error1.error.includes('401'));

      const error2: { status: number; error?: string } = {
        status: 401,
        error: 'Unauthorized',
      };
      const is401_2 =
        error2.status === 401 || (error2.error && error2.error.includes('401'));

      const error3: { status: number; error?: string } = {
        status: 500,
        error: 'Error 401',
      };
      const is401_3 =
        error3.status === 401 || (error3.error && error3.error.includes('401'));
